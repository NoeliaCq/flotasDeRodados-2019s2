 ...
 class Dependencia {
 	...
 	//método que recorre la flota de rodados de la dependencia y filtra aquellos que pueden realizar el 
 	//pedido pasado como parámetro
 	method quienesPuedenHacerPedido(pedido) {
 		return flota.filter({auto => auto.puedeHacerPedido(pedido)}) //se agregó este method a cada rodado
 	}
 	
 	//método que recorre la lista de pedidos de la dependencia y, utilizando el método de arriba, 
 	//identifica qué pedidos no pueden ser satisfechos por ningun rodado de la flota
 	method pedidosImposibles() {
 		return pedidos.filter({pedido=> self.quienesPuedenHacerPedido(pedido).size() == 0})
 	}
